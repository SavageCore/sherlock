...
def loadpage(url):
    url = "https://passtheheadphones.me/" + url
...
def pull_and_store(url, cursor):
    ...
    for child in children:
        ...
        statstuple = (str(statary[2]), str(float(statary[0]) / float(statary[1])), str(statary[1]), 'B',
                      str(statary[0]), 'B',  str(
                          long(statary[0]) - long(statary[1])), 'B',
...
for idset in ids:
    idstr = '|'.join(idset)
    url = '%s?users=%s&stats=uploaded|downloaded|username|seeding|leeching|snatched|uploads|posts' % (
        api_path, idstr)
